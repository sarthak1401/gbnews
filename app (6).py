...
date_period_options = ['All Time', 'Last 24 Hours', 'Last 7 Days', 'Last 30 Days', 'Last Year']
date_period = st.sidebar.selectbox("Date Period", date_period_options, index=0, key='date_period_selector')

sort_order_options = ['Latest to Oldest', 'Oldest to Latest']
sort_order = st.sidebar.selectbox("Sort Order", sort_order_options, index=0, key='sort_order_selector')
...
if not filtered_df.empty:
    for index, row in filtered_df.iterrows():
        ...
        st.caption(f"Source: {row['source']} | Date: {row['date'].strftime('%Y-%m-%d')} | Region: {row['region']}")
